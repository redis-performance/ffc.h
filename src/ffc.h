...
// noinline is load-bearing (unlike EXP-060, which annotated already-out-of-line
// code): GCC -O3 IPA otherwise re-inlines this single static caller. Forcing it
// out shrinks ffc_from_chars_double 4320->3395 B (-21%) and gives +3-10% on
// GCC. Gated to GCC: Clang (esp. AArch64, with its 2x-unroll/asm hot path)
// inlines differently and the outline regressed it ~1-3%, so Clang/MSVC keep
// the original inline form below (byte-identical to baseline).
...
ffc_adjusted_mantissa ffc_resolve_slow(ffc_parsed const pns, ffc_adjusted_mantissa am, ffc_value_kind vk) {
  ...
  // invalid power (am.power2 < 0) -> the long way around (big-integer compare)
  if (am.power2 < 0) {
...
ffc_result ffc_from_chars_advanced(ffc_parsed const pns, ffc_value* value, ffc_value_kind vk) {
  ...
  // its bytes stay out of this force-inlined hot frame (+3-10%); Clang/MSVC use
  // the original inline form (their inliners do better with it). Identical
  // results either way — the common case skips it entirely.
#if defined(__GNUC__) && !defined(__clang__)
  if (pns.too_many_digits || am.power2 < 0) {
